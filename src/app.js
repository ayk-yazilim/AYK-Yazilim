...
// Electron V3 uyumluluk katmanı
function storageKey(name){return 'AYK_MuhasebeYardimcisi_'+name;}
function readRegistry(name,defaultValue){
  try{var value=localStorage.getItem(storageKey(name));return value===null?defaultValue:value;}catch(e){return defaultValue;}
}
function writeRegistry(name,value,type){localStorage.setItem(storageKey(name),String(value));}
function deleteRegistryValue(name){try{localStorage.removeItem(storageKey(name));}catch(e){}}
function copyText(text,msg){
  if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).then(function(){alert(msg);}).catch(function(){fallbackCopy(text,msg);});}
...
function setAutoStart(enabled){
  if(!window.ayk||!window.ayk.setAutoStart)return;
  window.ayk.setAutoStart(!!enabled).then(function(ok){refreshStartupStatus();var e=document.getElementById('startupSaved');if(e){e.style.display='block';e.innerText=ok?(enabled?'Windows başlangıcında otomatik açılacak.':'Otomatik başlatma kapatıldı.'):'Ayar değiştirilemedi.';setTimeout(function(){e.style.display='none';},2200);}});
}
